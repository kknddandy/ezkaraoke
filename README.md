...
- VLC media player (video decoding; `sudo apt install vlc`)
...
sudo apt install ./dist/ezkaraoke_0.1.1_amd64.deb
...
User data (not part of the package):

- config: `~/.config/ezkaraoke/config.json` (music folder, language)
- song library: `~/.local/share/ezkaraoke/`
...
6. Language: the toolbar button (shows the *other* language) switches
   the whole UI between Chinese and English instantly; the choice is
   saved and restored on the next start.

## Video output

- **X11 / XWayland** — hardware-accelerated direct window output
  (`set_hwnd`).
...
If the app reports that VLC is missing, make sure `vlc` is installed
...
ezkaraoke/
  ...
  config.py          JSON config (music folder, language)
packaging/           .deb build script, desktop entry, icons
...
(`packaging/vendor/`, MIT) because Ubuntu does not package it.
...
用户数据（不在包内）：配置 `~/.config/ezkaraoke/config.json`（音乐
文件夹、语言），曲库 `~/.local/share/ezkaraoke/`。
...
5. 播放器窗口：
   ...
   - **原唱/伴奏** 切换双音轨视频的音轨
6. 语言：工具栏按钮显示"另一种语言"（中文模式下显示 EN），点击即时
   切换整个界面，选择会被保存并在下次启动时恢复。

## 视频输出策略

- **X11 / XWayland**：窗口句柄硬件加速直出（`set_hwnd`）。
- **原生 Wayland**：自动切换软件渲染模式（VLC 解码后回读帧到 Qt
  显示，CPU 占用略高，功能完整）。

若提示缺少 VLC，请确保系统已安装 VLC 且命令行可用。
...
ezkaraoke/
  ...
  i18n.py            中/英界面翻译
  config.py          JSON 配置（音乐文件夹、语言）
packaging/           .deb 构建脚本、桌面条目、图标
...
（`packaging/vendor/`，MIT），因为 Ubuntu 未打包该库。
